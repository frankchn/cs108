...
public class RelationManager {
	...
	public static boolean friends(int user_id1, int user_id2) {
		try {
			PreparedStatement p = db.prepareStatement("SELECT * FROM `friend` WHERE `user_id_1` =" + user_id1 + " AND `user_id_2`=" + user_id2);
			ResultSet r = p.executeQuery();
	...
	public static boolean removeFriendRequest(int requestor_id, int requestee_id) {
		try {
			return db.prepareStatement("DELETE FROM `user` WHERE `requestor_user_id` =" + requestor_id + " AND `requestee_user_id` =" + requestee_id).execute();
		} catch (SQLException e) {
			return false;
		}
	}
